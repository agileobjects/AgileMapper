    ...
    public void ShouldIncludeADataSourceFuncInvocation()
    {
        using (var mapper = Mapper.CreateNew())
        {
            ...
            string mappingPlan = mapper
                .GetPlanFor<Issue168.ISomething>()
                .ToANew<Issue168.TgtSomething>();

            mappingPlan.ShouldNotBeNull();

            mappingPlan = mappingPlan.RemoveWhiteSpace();

            mappingPlan.ShouldContain("switch(Func<");
            mappingPlan.ShouldContain(">.Invoke(");
            mappingPlan.ShouldContain("context.SetTarget(tgtSomething).AddChild(");
        }
    }
